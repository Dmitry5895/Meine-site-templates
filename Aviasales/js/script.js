...
const cityesApi = 'http://api.travelpayouts.com/data/ru/cities.json',
    ...
    API_KEY = '37ae04d4f26c830628cf98bd0e36a5e7',
...
formSearch.addEventListener('submit', (event) => {
    ...
    if (formData.from && formData.to) {
        ...
        getData(calendar + requestData, (response) => {
            ...
            cheapestTicket.style.display = 'block';
            cheapestTicket.innerHTML = '<h2>На данный рейс нет отправлений!</h2>';
            console.error('Ошибка', error)
        });

    } else {
        otherCheapTickets.style.display = 'block';
        cheapestTicket.innerHTML = '<h2>Город указан не корректно</h2>';
    };
